(function () {
  ...
  function render() {
    ...
    rows.forEach(function (it) {
      ...
      votes.textContent = n + (n === 1 ? " vote" : " votes");
      ...
      who.textContent = "Intended users: " + it.for;
      ...
      btn.disabled = busy;
      btn.textContent = mine.votes[it.id] ? "Remove my vote" : "I think this is needed";
    ...
    var mode = endpoint
      ? "Votes are counted using anonymous IDs. You may cast one vote per idea in the same browser."
      : "Votes are saved in this browser. Combined totals from all users will be enabled after the voting script for the operations sheet is published.";
    tally.textContent = (nMine
      ? "You have voted for " + nMine + (nMine === 1 ? " idea." : " ideas.")
      : "You have not voted yet.") + " " + mode;

    var ranked = ideas.slice().sort(function (a, b) { return countFor(b.id) - countFor(a.id); });
    if (ranked[0] && countFor(ranked[0].id) > 0) {
      var topCount = countFor(ranked[0].id);
      top.hidden = false;
      top.textContent = "Most votes now: " + ranked[0].title + " (" + topCount +
  ...
  bindFilters();
  Promise.all([
    fetch("../../data/feature-ideas-en.json").then(function (r) { return r.json(); }),
    ...
    document.getElementById("empty").textContent = "The ideas could not be loaded.";
  });
})();
